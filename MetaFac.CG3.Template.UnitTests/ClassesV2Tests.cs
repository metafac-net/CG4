...
namespace MetaFac.CG3.Template.UnitTests
{
    ...
    public class ClassesV2Tests
    {
        ...
        public void Create_NonEmpty()
        {
            ...
            original.Freeze();
            original.IsFrozen().Should().BeTrue();
            original.T_UnaryModelFieldName_.IsFrozen().Should().BeTrue();
            original.T_ArrayModelFieldName_[0]?.IsFrozen().Should().BeTrue();
            original.T_IndexModelFieldName_["987"]?.IsFrozen().Should().BeTrue();
