...
namespace MetaFac.CG3.Generator.RecordsV2
{
    ...
    {
        public Generator() : base("MetaFac.CG3.RecordsV2") { }
...
Emit("");
    using (Ignored()) {
Emit("    public record T_BaseClassName_ : EntityBase, IT_BaseClassName_");
Emit("    {");
...
Emit("    public abstract partial record T_ClassName2_");
Emit("    {");
...
Emit("    public partial record T_ClassName_");
Emit("    {");
Emit("        [MethodImpl(MethodImplOptions.AggressiveInlining)]");
Emit("        public static T_ClassName_? CreateFrom(IT_ClassName_? source)");
Emit("        {");
...
Emit("    public partial record T_ClassName_ : T_BaseClassName_, IT_ClassName_");
Emit("    {");
        using (Ignored()) {
...
Emit("        IReadOnlyList<T_ExternalOtherType_?>? IT_ClassName_.T_ArrayMaybeFieldName_ => T_ArrayMaybeFieldName_;");
...
Emit("        IReadOnlyDictionary<T_IndexType_, T_ExternalOtherType_?>? IT_ClassName_.T_IndexMaybeFieldName_ => T_IndexMaybeFieldName_;");
...
Emit("        IReadOnlyList<T_ExternalOtherType_>? IT_ClassName_.T_ArrayOtherFieldName_ => T_ArrayOtherFieldName_;");
...
Emit("        public ReadOnlyMemory<byte> T_UnaryBufferFieldName_ { get; init; }");
Emit("        ReadOnlyMemory<byte> IT_ClassName_.T_UnaryBufferFieldName_ => T_UnaryBufferFieldName_;");
            break; case FieldKind.ArrayBuffer:
Emit("        public ImmutableList<ReadOnlyMemory<byte>>? T_ArrayBufferFieldName_ { get; init; }");
Emit("        IReadOnlyList<ReadOnlyMemory<byte>>? IT_ClassName_.T_ArrayBufferFieldName_ => T_ArrayBufferFieldName_ is null");
Emit("            ? null");
Emit("            : new ListFacade<ReadOnlyMemory<byte>, ReadOnlyMemory<byte>>(T_ArrayBufferFieldName_, (x) => x);");
            break; case FieldKind.IndexBuffer:
Emit("        public ImmutableDictionary<T_IndexType_, ReadOnlyMemory<byte>>? T_IndexBufferFieldName_ { get; init; }");
Emit("        IReadOnlyDictionary<T_IndexType_, ReadOnlyMemory<byte>>? IT_ClassName_.T_IndexBufferFieldName_ => T_IndexBufferFieldName_ is null");
Emit("            ? null");
Emit("            : new DictionaryFacade<T_IndexType_, ReadOnlyMemory<byte>, ReadOnlyMemory<byte>>(T_IndexBufferFieldName_, (x) => x);");
...
Emit("                    .Select(x => (ReadOnlyMemory<byte>)x));");
                break; case FieldKind.IndexBuffer:
...
Emit("                    .Select(x => new KeyValuePair<T_IndexType_, ReadOnlyMemory<byte>>(x.Key, x.Value)));");
                break; case FieldKind.UnaryString:
Emit("            T_UnaryStringFieldName_ = source.T_UnaryStringFieldName_;");
                break; case FieldKind.ArrayString:
...
Emit("");
    using (Ignored()) {
Emit("    public record T_SuperClassName_ : T_ClassName_, IT_SuperClassName_");
Emit("    {");
